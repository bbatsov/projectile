...
It's important to note that at the moment the `.projectile` file is
consulted in both modes, but ignored files are taken into account only
when using `native indexing`. Ignored folders are always taken into account.

This is intentional, since applying any filtering on the results
generated by external commands would impact performance greatly and
would that way eliminate its advantage - that it is so fast one
