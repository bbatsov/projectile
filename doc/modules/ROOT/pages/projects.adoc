...
(a `+` between markers means all of them have to be present, `or` means any one of them will do):
...
| `bazel`
| `MODULE.bazel` or `WORKSPACE` or `WORKSPACE.bazel`
...
| `docker-compose`
| `compose.yaml` or `compose.yml` or `docker-compose.yaml` or `docker-compose.yml`

| `dotnet`
| `projectile-dotnet-project-p` (predicate)

| `dotnet-sln`
| `projectile-dotnet-sln-project-p` (predicate)
...
| `go`
| `projectile-go-project-p` (predicate)

| `go-task`
| `Taskfile.yml` or `Taskfile.yaml` or `Taskfile.dist.yml` or `Taskfile.dist.yaml`

| `godot`
| `project.godot`

| `gradle`
| `build.gradle` or `build.gradle.kts` or `settings.gradle` or `settings.gradle.kts`
...
| `haskell-cabal`
| `projectile-cabal-project-p` (predicate)
...
| `hugo`
| `hugo.toml` or `hugo.yaml` or `hugo.json`
...
| `just`
| `justfile` or `.justfile` or `Justfile`
...
| `make`
| `projectile-make-project-p` (predicate)
...
| `mill`
| `projectile-mill-project-p` (predicate)
...
| `php-symfony`
| `composer.json` + (`bin/console` or `app/console`)
...
| `terraform`
| `projectile-terraform-project-p` (predicate)

| `turborepo`
| `turbo.json`

| `xcode`
| `projectile-xcode-project-p` (predicate)
...
Plenty of projects match more than one of these - a Django application has a
`pyproject.toml`, a Laravel one has a `package.json` for its assets, and all sorts of
projects ship a `justfile` or a Compose file. Projectile tries the types from the most
specific to the least, so the framework wins over the packaging tool, and the packaging
tool wins over the generic task runner. `projectile-doctor` reports which type matched
and on which marker, which is the quickest way to find out why a project came out the
way it did.
...
. add a list of files and/or folders in a root of the project that helps to identify the type, in this case it is only `package.json`. When you list several markers, *all* of them must be present for the type to match (logical AND). A marker can also be an alternatives clause of the form `(:any "file1" "file2")`, which matches when *any* one of its files is present - that's how a type covers a build file with several spellings, e.g. `'((:any "build.gradle" "build.gradle.kts"))`. For anything more involved the marker list can be replaced by a function which takes a project root as argument and returns non-nil when that directory has the correct project structure for the type.
