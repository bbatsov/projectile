...
* **Switching projects.** `project-switch-project` opens a command menu; Projectile instead runs whatever's in `projectile-switch-project-action` after picking a project (find-file by default). If you miss the menu UX, press kbd:[C-u s-p p] to get the `projectile-dispatch` menu after selecting a project (requires the optional `transient` dependency).
