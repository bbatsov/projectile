...
menu than memorize a lot of keybindings. Invoke it with kbd:[s-p m].
...
`projectile-dispatch` is only available when `transient` is installed. Without
it kbd:[C-u s-p p] simply falls back to `projectile-switch-project-action`.
