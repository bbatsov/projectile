...
image::projectile-discovered-tasks.png[projectile-run-task offering npm scripts and Makefile targets discovered in the project]
...
+
image::projectile-find-file-in-subproject.png["The subproject prompt, listing the parts of a monorepo"]
...
The cursor decides which test runs - here it sits in the second of the
file's two tests, and only that one is handed to `go test`:

image::projectile-test-at-point.gif["With the cursor inside one test function, projectile-run-test-at-point runs just that test and the compilation buffer reports it passing"]
