...
* jump to a file in project, xref:configuration.adoc#file-ranking-frecency[ranked by frecency]
...
* xref:configuration.adoc#per-project-sessions[per-project sessions] with tab-bar workspaces (restore each project's layout and buffers, even across restarts)
...
* run xref:projects.adoc#project-tasks[project commands and tasks] (build/test/run, `make`, and your own named tasks)
* xref:projects.adoc#running-the-test-at-point[run the test at point] (tree-sitter, Emacs 29+)
