...
`projectile-ignored-project-regexps` takes regexps rather than exact paths,
which is usually what you want for whole areas of a machine:

[source,elisp]
----
;; Never remember anything under /tmp or in a Downloads directory
(setq projectile-ignored-project-regexps '("\\`/tmp/" "/Downloads/"))
----
...
[source,elisp]
----
(setq projectile-auto-track-known-projects nil)
----
...
Those messages are prefixed with `+[Projectile]+`, so it's clear who is talking
when you didn't ask anything of it. Direct answers to a command you just
invoked go unprefixed - you know who is talking, and the echo area is narrow -
except for the ones that arrive later out of a background process, which are
prefixed but never suppressed.
