...
* Under `alien` and `hybrid` indexing the VCS is consulted too, so a
  watched project doesn't gain files your `.gitignore` excludes. That's one
  `git check-ignore` per batch of events rather than one per file, since a
  batch can cover a whole directory that was moved in. Other version control
  systems aren't asked, and `native` indexing lists such files anyway.
