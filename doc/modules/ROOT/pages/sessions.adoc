...
Both halves of that in one go - switching away opens a tab of its own,
switching back selects the tab that was already there, layout and all:

image::projectile-sessions.gif["Switching from one project to another opens a second tab, and switching back selects the first tab again with its two-window layout intact"]
...
`projectile-session-auto-save` (on by default) does the saving for you: the
...
[source,elisp]
----
(setq projectile-session-auto-save nil)
----
