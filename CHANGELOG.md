...
* [#2046](https://github.com/bbatsov/projectile/issues/2046): Commands chosen from the `projectile-dispatch` menu opened via `C-u s-p p` now run in the project that was just selected, instead of the current buffer's project. The menu is a `transient`, so its suffix commands run after the project switch has unwound; the switched-to project is now kept current for the lifetime of the menu and restored when it exits.
