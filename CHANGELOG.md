...
- [#2123](https://github.com/bbatsov/projectile/pull/2123): Every lifecycle command now has a subproject variant - `projectile-configure-subproject`, `-install-` and `-package-` join compile, test and run, bound one level down from the project-wide keys (`s-p c m o`, `s-p c m i`, `s-p c m p`).
- [#2123](https://github.com/bbatsov/projectile/pull/2123): Fix the subproject commands discarding the project type's compilation directory: they replaced `projectile-project-compilation-dir` with the subproject's path, so in a Meson monorepo `projectile-compile-subproject` ran ninja in `sub/` instead of `sub/build/`. The subproject is now the directory the type's compilation directory is resolved against, which `projectile-compilation-dir` takes as an optional argument.
- [#2123](https://github.com/bbatsov/projectile/pull/2123): The subproject commands say where they'll run at the prompt (`Test command in services/api/:`), so a subproject build isn't mistaken for a whole-project one.
...
- [#1075](https://github.com/bbatsov/projectile/issues/1075): Opening a file the VCS ignores no longer adds it to the project's file cache under `alien`/`hybrid` indexing, where the file list comes from the VCS - so a compiled artifact you happen to visit stops turning up in `projectile-find-file`. Projectile's own ignore rules can't answer for `.gitignore`, so git is asked directly (`projectile-vcs-ignored-file-p`); other version control systems answer nil, as before. Newly created files that aren't ignored are still cached on open, and `native` indexing is untouched, since it lists such files anyway.
